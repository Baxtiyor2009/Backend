...
# ── Admin direct token (for admin panel login) ────────────────────────────────
...
async def get_admin_token(
    ...
    """
    Issue JWT token for admin panel login.
    Only works if telegram_id is in ADMIN_IDS env variable.
    """
    from api.middleware.auth import create_access_token

    if body.telegram_id not in settings.admin_id_list:
        raise HTTPException(status_code=403, detail="Ruxsat yoq — ADMIN_IDS da emas")

    result = await db.execute(select(User).where(User.telegram_id == body.telegram_id))
    user = result.scalar_one_or_none()

    if not user:
        raise HTTPException(status_code=404, detail="Foydalanuvchi topilmadi. Avval botga /start yuboring.")
